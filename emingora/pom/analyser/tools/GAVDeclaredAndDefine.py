...
class GAVDeclaredAndDefine:
    ...
    def get_repeated_gavs(pom: Pom, found_dict: {} = {}, inherited_dep_management: [] = None) -> {}:
        ...
        for dependency in dependencies_management:
            GAVDeclaredAndDefine.check(dependency, pom.dependencies, found_dict)
            GAVDeclaredAndDefine.check_children(pom, dependency, dependencies_management, found_dict)

        GAVDeclaredAndDefine.check_parents(pom.dependencies_management, pom.parent, found_dict)
        return found_dict

    @staticmethod
    def check(dependency: GAV, dependencies: [], found_dict: {}):
        for dep in dependencies if dependencies is not None else []:
            if GAVUtils.is_gav_equal_not_none_version(dependency, dep):
                GAVDeclaredAndDefine.fill_found_dict(dependency, dep, found_dict)

    @staticmethod
    def check_children(pom: Pom, dependency: GAV, dependencies_management: [], found_dict: {}):
        if pom.children is not None:
            for children in pom.children:
                GAVDeclaredAndDefine.check(dependency, children.dependencies, found_dict)
                GAVDeclaredAndDefine.check(dependency, children.dependencies_management, found_dict)
            for child in pom.children:
                GAVDeclaredAndDefine.get_repeated_gavs(child, found_dict, dependencies_management)

    @staticmethod
    def check_parents(dependencies_management: [], parent_pom: Pom, found_dict: {}):
        if parent_pom is not None:
            for dependency_management in dependencies_management if dependencies_management is not None and parent_pom.dependencies is not None else []:
                GAVDeclaredAndDefine.check(dependency_management, parent_pom.dependencies, found_dict)
            if parent_pom.parent is not None:
                GAVDeclaredAndDefine.check_parents(dependencies_management, parent_pom.parent, found_dict)

    @staticmethod
    def fill_found_dict(dependency1: GAV, dependency2: GAV, found_dict: {}):
